...
OK, but patterns are the key of graph databases. Add pattern-names as documentation to above statement and use readable node identifiers.
[source,cypher]
----
...
This will show all paths (2 paths with two nodes, one path with 3 nodes). We need to find full (longest) path.
...
=== Merge participants
For the Bollekesloop, I want to add the participants to the Main race (10k). Merge is used to create or match each participant. 
...
Sequence of arrival is determined using the 'after' relation. At the end of a race, this is the only known attribute and that is sufficient to calculate the new overall ranking. Later on when more information (e.g. place of each finisher in the race) comes available, this can be added to the database.
