...
Sometimes people come and join to run the race, without being part of the competition. This is the case for people interested in joining the club next year (hang-arounds in Hells Angels terminology), or people who are a bit late with membership fee. We want to remember their race result to display in the weekly Club Gazette or to change their status to participant and recalculate points as soon as they are a club member. 
...
Sequence of arrival is determined using the 'after' relation. At the end of a race, this is the only known attribute and that is sufficient to calculate the new overall ranking. Later on when more information (e.g. place of each finisher in the race) is available, this can be added in the database. This datamodel is proof of concept to start building the web application in Flask.
